...
OUTPUT_CSV_ANALYSIS = lambda target_date : f"output/smhi/smhi_analysis_{target_date}.csv"
...
def fetch_weather_for_date(param, station_meta, obs_url_lambda, target_date, entire_year=False, 
                           stations_list_csv_path="weather_stations.csv", desired_timezone="Europe/Stockholm"):
...
def analyze_results(csv_path_lambda, param_descs, meteo_params, target_date):
...
def plot_results(df_all, stations_list_csv_path="weather_stations.csv"):
...
# Test function to get an overview of the weather data
def test():
    ...
    for param in meteo_params:
        #weather_data = fetch_weather_for_date(param, get_all_stations(STATIONS_URL(param)), OBS_URL, target_date=target_date, entire_year=entire_year)
    ...
    merged_raw = merge_raw_weather_files(OUTPUT_CSV_RAW, param_descs, meteo_params, target_date)
    merged_raw.write_csv(OUTPUT_CSV_MERGED(target_date))
    print(f"Wrote merged weather data to {OUTPUT_CSV_MERGED(target_date)}")

    # Analysis and plotting
    #df_all, metrics = analyze_results(OUTPUT_CSV_RAW, param_descs, METEO_PARAMS, target_date)
    #print(metrics)
